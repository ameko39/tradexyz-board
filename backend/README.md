...
The AI calendar is generated by the backend, cached on disk, and refreshed once per day at 12:00 Beijing time. Fetching raw calendar/TG data does not use AI tokens; only the generation step uses `AI_API_KEY`. OpenAI-compatible `OPENAI_API_KEY`, `OPENAI_MODEL`, and `OPENAI_BASE_URL` are still supported as fallback environment variable names.
